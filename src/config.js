      ...
      if (rule.site != '') {
        rule.site.split(',').forEach(site => {
          ...
          site_rule_sets.push({
            ...
            type: 'remote',
            format: 'binary',
            url: `${SITE_RULE_SET_BASE_URL}geosite-${s}.srs`,
          });
        });
      }
      if (rule.ip != '') {
        rule.ip.split(',').forEach(ip => {
          ...
          ip_rule_sets.push({
            ...
            type: 'remote',
            format: 'binary',
            url: `${IP_RULE_SET_BASE_URL}geoip-${i}.srs`,
          });
        });
      }
    });
...
// 你的其他配置（dns, clash, surge等）保持不变
// ...（省略部分配置，为了节省空间）...

// 例子：优化的`addNodeSelectGroup()`方法
export class ConfigBuilder {
  ...
  addNodeSelectGroup(proxyList) {
    // 避免重复加入“直连”
    const hasDirect = proxyList.some(node => node.name === '直连' && node.type === 'direct');
    if (!hasDirect) {
      proxyList.push({ name: '直连', type: 'direct' });
    }

    // 避免重复节点
    proxyList = proxyList.filter((node, index, self) => {
      return index === self.findIndex((t) => t.name === node.name && t.type === node.type);
    });
...
} const CLASH_IP_RULE_SET_BASE_URL = 'https://gh-proxy.com/https://github.com/MetaCubeX/meta-rules-dat/raw/refs/heads/meta/geo/geoip/';
export const SURGE_SITE_RULE_SET_BASEURL = 'https://gh-proxy.com/https://github.com/NSZA156/surge-geox-rules/raw/refs/heads/release/geo/geosite/'
export const SURGE_IP_RULE_SET_BASEURL = 'https://gh-proxy.com/https://github.com/NSZA156/surge-geox-rules/raw/refs/heads/release/geo/geoip/'
// Custom rules
export const CUSTOM_RULES = [];
