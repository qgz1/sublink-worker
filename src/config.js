...
// 基础规则结构（不包含 outbound 映射）
const UNIFIED_RULES_BASE = [
  {
    name: 'Ad Block',
...
// 完整的 UNIFIED_RULES（应用代理组映射）
export const UNIFIED_RULES = UNIFIED_RULES_BASE.map(rule => ({
  ...rule,
...
// Generate SITE_RULE_SETS and IP_RULE_SETS from UNIFIED_RULES_BASE
...
export const CLASH_SITE_RULE_SETS = UNIFIED_RULES_BASE.reduce((acc, rule) => {
  rule.site_rules.forEach(site_rule => {
...
export const CLASH_IP_RULE_SETS = UNIFIED_RULES_BASE.reduce((acc, rule) => {
  rule.ip_rules.forEach(ip_rule => {
...
export function generateRuleSets(selectedRules = [], customRules = []) {
  ...
  UNIFIED_RULES_BASE.forEach(rule => {
