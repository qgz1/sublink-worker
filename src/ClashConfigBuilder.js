...
import { Bimport yaml from 'js-yaml';
import { CLASH_CONFIG, generateRules, generateClashRuleSets, getOutbounds, PREDEFINED_RULE_SETS } from './config.js';
        ...
        outbounds.forEach(outbound => {
            ...
            if (outboundName !== nodeSelect) {
                ...
                if (outboundName === t('outboundNames.国内服务') || outboundName === '🔒 国内服务') {
                    ...
                    // 🎥 流媒体服务：优先低延迟节点
                    if (/media|stream|video|youtube|netflix|disney|hbo|hulu|bilibili|twitch/i.test(outbound)) {
                        optimized = new Set([
                            nodeSelect,
                            autoSelect,
                            ...proxyList.filter(name => /香港|新加坡|日本|美国|hk|sg|jp|us/i.test(name)),
                            'DIRECT',
                            'REJECT'
                        ]);
                    } 
                    // 🤖 AI服务：优先支持OpenAI的节点
                    else if (/openai|chatgpt|ai|gemini|claude|copilot/i.test(outbound)) {
                        optimized = new Set([
                            nodeSelect,
                            autoSelect,
                            ...proxyList.filter(name => /新加坡|美国|日本|英国|sg|us|jp|uk/i.test(name)),
                            'DIRECT',
                            'REJECT'
                        ]);
                    }
                    // 🎮 游戏服务：极低延迟优先
                    else if (/game|gaming|steam|battle|riot|epic|origin/i.test(outbound)) {
                        optimized = new Set([
                            nodeSelect,
                            autoSelect,
                            ...proxyList.filter(name => /香港|日本|韩国|新加坡|hk|jp|kr|sg/i.test(name)),
                            'DIRECT',
                            'REJECT'
                        ]);
                    }
                    // 📥 下载服务：高带宽优先
                    else if (/download|torrent|p2p|bt|emule/i.test(outbound)) {
                        optimized = new Set([
                            nodeSelect,
                            autoSelect,
                            ...proxyList.filter(name => /新加坡|日本|美国|德国|sg|jp|us|de/i.test(name)),
                            'DIRECT',
                            'REJECT'
                        ]);
                    }
                    // 💬 社交媒体：稳定连接优先
                    else if (/social|twitter|facebook|instagram|telegram|discord|whatsapp/i.test(outbound)) {
                        optimized = new Set([
                            nodeSelect,
                            autoSelect,
                            ...proxyList.filter(name => /香港|新加坡|日本|hk|sg|jp/i.test(name)),
                            'DIRECT',
                            'REJECT'
                        ]);
                    }
...
}aseConfigBuilder } from './BaseConfigBuilder.js';
import { DeepCopy } from './utils.js';
import { t } from './i18n/index.js';
